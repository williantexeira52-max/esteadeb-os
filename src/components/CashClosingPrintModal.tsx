...
export const CashClosingPrintModal: React.FC<CashClosingPrintModalProps> = ({ isOpen, onClose }) => {
  ...
  const entriesCount = transactions.filter(t => t.type === 'ENTRY' || t.amount > 0).reduce((acc, curr) => acc + Number(curr.amount || 0), 0);
                      ...
                      <tbody>
                        {transactions.map((t, idx) => (
                           <tr key={idx} className="border-b border-slate-100">
                             ...
                             <td className={cn("p-2 text-right font-black", (t.type === 'ENTRY' || t.amount > 0) ? 'text-emerald-600' : 'text-red-600')}>
                               {t.type === 'WITHDRAWAL' || t.amount < 0 ? '-' : ''}R$ {Math.abs(Number(t.amount || 0)).toFixed(2)}
                             </td>
                           </tr>
                        ))}
                      </tbody>
